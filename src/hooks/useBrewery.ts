...
export function useActiveBatches(): { tanks: Tank[]; batches: (ActiveBatch | null)[] } {
  ...
  const batches = tanks.map((tank, i) => {
    ...
    if (composed) {
      ...
      // (fallen sideways, in foam/CO₂, or lifted out). composeBatch already nulled
      // the gravity-derived tiles; here we surface WHY as an alert so the card
      // explains the '—'s instead of silently blanking.
      if (composed.gravitySuspect && !alerts.some((x) => x.key === 'gravity_suspect')) {
        alerts.push({
          key: 'gravity_suspect', severity: 'problem', label: 'GRAVITY READING SUSPECT',
          entityId: 'app:gravitySuspect', detail: composed.gravitySuspectReason ?? undefined,
        });
      }
